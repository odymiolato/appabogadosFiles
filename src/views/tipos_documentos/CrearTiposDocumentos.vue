...
import { tipos_documentos } from '../../class/all.class'
...
const tipo = ref<tipos_documentos>(new tipos_documentos())
...
async function fetchCliente(id: string) {
  try {
    ...
    addAlert(1, 'Tipo cargado correctamente.')
  }
  catch (error) {
    console.error('Error fetching Tipo:', error)
    addAlert(3, 'Error al obtener el Tipo.')
  }
}
...
async function saveTipoDocumento() {
  try {
    if (route.params.id) {
      ...
      await apiClient.patch(`/tiposdocumentos/${route.params.id}`, tipo.value)
      addAlert(2, 'El tipo se actualizó correctamente.')
    }
    else {
      // Crear nuevo cliente
      await apiClient.post('/tiposdocumentos', tipo.value)
      addAlert(2, 'El tipo se registró correctamente.')
    }
    router.push({ name: 'Tipos Documentos' })
  }
  catch (error) {
  ...
  <div class="mt-4 p-6 bg-white rounded-md shadow-md">
    ...
    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
      <div>
        ...
        <input id="nombre" v-model="tipo.nombre_tdoc" type="text"
          class="w-full mt-2 border-gray-200 rounded-md focus:border-sky-600 focus:ring focus:ring-opacity-40 focus:ring-sky-500">
      </div>
      <div>
        <label class="text-gray-700" for="apellido">Descripción</label>
        <input id="apellido" v-model="tipo.descri_tdoc" type="text"
          class="w-full mt-2 border-gray-200 rounded-md focus:border-sky-600 focus:ring focus:ring-opacity-40 focus:ring-sky-500">
      </div>
