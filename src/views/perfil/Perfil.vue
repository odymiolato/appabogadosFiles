...
// Cargar programas desde el servidor al montar el componente
onMounted(async () => {
    try {
...
function updateProgramOption(programId: number, key: keyof modulo_programa, value: boolean) {
...
async function saveModule() {
    ...
    try {
        const response = await fetch(`${URL}modulos`, {
            ...
            body: JSON.stringify({
                modulo: modulo.value,
                programas: selectedPrograms.value,  // Asociamos los programas seleccionados
            }),
        })
        ...
        if (response.ok) {
            ...
            selectedPrograms.value = []  // Limpiar la selección
        } else {
            console.error('Error en la respuesta del servidor:', response.statusText)
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="">
                                <tr v-for="(value) in availablePrograms" :key="value.codpro_pro"
                                    ...
                                    <td class="px-6 py-4 text-lg text-gray-700 border-b">
                                        <input type="checkbox"
                                            @change="e => updateProgramOption(value.codpro_pro, 'insertar_mop', e.target.checked)">
                                    </td>
                                    <td class="px-6 py-4 text-lg text-gray-700 border-b">
                                        <input type="checkbox"
                                            @change="e => updateProgramOption(value.codpro_pro, 'eliminar_mop', e.target.checked)">
                                    </td>
                                    <td class="px-6 py-4 text-lg text-gray-700 border-b">
                                        <input type="checkbox"
                                            @change="e => updateProgramOption(value.codpro_pro, 'modificar_mop', e.target.checked)">
                                    </td>
                                </tr>
                            </tbody>
                        </table>
